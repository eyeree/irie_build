...
func delegate_evaluation(command: String, env: ReplEnv):
	...
	var ext = ReplExt.new()
	var num_object_props = Object.new().get_property_list().size()
	var ext_props = ext.get_property_list().slice(num_object_props)
	for ext_prop in ext_props:
		env_keys.append(ext_prop.name)
		env_values.append(ext.get(ext_prop.name)) 
